...
import { Mail, Phone, User, School, Send, CheckCircle, AlertCircle } from 'lucide-react';
...
const Contact: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus({ type: 'success', message: 'Pesan Anda berhasil terkirim. Terima kasih!' });
      setFormData({ name: '', email: '', subject: '', message: '' });
    } catch (error: any) {
      setStatus({ type: 'error', message: error.message || 'Gagal mengirim pesan, coba lagi nanti.' });
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center group">
                  ...
                  <div>
                    ...
                    <a href="mailto:fatih.rizqi@example.com" className="text-slate-900 font-medium hover:text-blue-600">fatih.rizqy107@gmail.com</a>
                  </div>
                </div>
                ...
                <div className="flex items-center group">
                  ...
                  <div>
                    ...
                    <p className="text-slate-900 font-medium">0819-3118-6752</p>
                  </div>
                </div>
          ...
          <div className="lg:col-span-7">
            <div className="bg-white rounded-2xl shadow-lg border border-slate-100 p-8 lg:p-10 h-full">
              ...
              {status.type && (
                <div className={`mb-6 p-4 rounded-xl flex items-start ${status.type === 'success' ? 'bg-green-50 text-green-800 border border-green-200' : 'bg-red-50 text-red-800 border border-red-200'
                  }`}>
                  {status.type === 'success' ? (
              ...
              <form className="space-y-6" onSubmit={handleSubmit}>
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className={`w-full text-white font-bold py-4 rounded-xl shadow-lg flex justify-center items-center transition-all ${loading ? 'bg-slate-500 cursor-not-allowed' : 'bg-slate-900 hover:bg-blue-600 hover:shadow-blue-500/25'
                    }`}
                >
                  {loading ? (
